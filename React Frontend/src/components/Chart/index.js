  ...
  getData() {
    ...
    if (span != undefined) {
      ...
      url = `https://min-api.cryptocompare.com/data/histo${
        conf.unit
      }?fsym=${fsym}&tsym=${tsym}&limit=${conf.limit}&aggregate=${
        conf.aggregate
      }`;
    } else {
      conf = {
  ...
  draw() {
    ...
    if (this.state.timezone != undefined) {
      offset =
        ...
        moment.tz.zone(this.state.timezone).parse(Date.UTC());
    }
    ...
    if (this.state.noData) {
      canvas = (
        <div style={{ height: `100%`, width: `100%` }}>
          <div style={{ marginTop: this.state.height * 0.4 }}>
            ${this.state.fsym}のデータはありません。
          </div>
        </div>
      );
    } else {
      canvas = (
        <canvas id={id} width={this.state.width} height={this.state.height} />
      );
    }
